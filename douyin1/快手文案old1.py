...
db = pymysql.connect(host='http://kdy1.qwxyx.xyz',
                             port=3306,
                             user='jk',
                             password='11223344',
...
for i0 in sz1:
    ...
    # r = requests.get(url='https://www.kuaishou.com/short-video/3x32yi3a5px89ys', headers=headers, stream=True)
        ...
        with open('E:one1t/OneDrive - 356h31/fenlei/YY1K/' +'a2-'+str(i1) + '.mp4', 'wb') as f:
            f.write(r.content)
            f.close()
        with open('E:one1t/OneDrive - 356h31/fenlei/YY1K/' + 'b2-' + str(i1) + '.jpg', 'wb') as f:
            f.write(r1.content)
            f.close()
